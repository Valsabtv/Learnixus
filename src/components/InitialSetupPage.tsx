...
interface InitialSetupPageProps {
  onSetupComplete: (name: string, studyLevel: string, college: string, examInfo: ExamPreparationInfo) => Promise<void>;
  currentName: string;
  currentLevel: string;
...
const InitialSetupPage: React.FC<InitialSetupPageProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      ...
      if (isPreparingForExam === 'yes') {
        if (examTypeCompetitive === 'yes') {
          if (!selectedCompetitiveExamKey) {
            addNotification('Please select the competitive exam.', 'warning');
            setIsSubmitting(false);
            return;
          }
          examInfoToSave = {
          ...
          addNotification('Please specify the type of major exam.', 'warning');
          setIsSubmitting(false);
          return;
        }
    ...
    try {
      await onSetupComplete(userNameInput.trim(), selectedLevel, finalSelectedCollege || '', examInfoToSave);
    } catch (error) {
      // Error is already handled in the App component's onSetupComplete
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const appNameColor = `text-${accentColor}-500`;
  const inputBg = `bg-slate-50 border-slate-300 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-slate-700 dark:border-slate-600 dark:focus:ring-sky-500 dark:focus:border-sky-500`;
  const inputTextColor = `text-slate-900 placeholder-slate-400 dark:text-slate-100 dark:placeholder-slate-400`;
  const labelColor = `text-slate-600 dark:text-slate-300`;
  ...
  const radioBorderColor = `border-slate-300 dark:border-slate-600`;
  const radioCheckedBorderColor = `border-${accentColor}-500 dark:border-${accentColor}-400`;
  const radioRingColor = `ring-${accentColor}-500 dark:ring-${accentColor}-400`;
  const dropdownBg = `bg-white dark:bg-slate-700`;
  const dropdownItemHoverBg = `hover:bg-indigo-50 dark:hover:bg-sky-600`;
  const subSectionBg = `bg-slate-50/70 dark:bg-slate-700/40`;
  const subSectionBorder = `border-slate-200 dark:border-slate-600`;
    ...
    <div className={`min-h-screen ${pageBg} flex flex-col items-center justify-center p-4 sm:p-6 transition-colors duration-300 ease-in-out animate-fadeIn`}>
      <div className={`w-full max-w-xl ${cardBg} p-6 sm:p-8 md:p-10 rounded-xl shadow-2xl`}>
        <div className="flex flex-col items-center mb-6 sm:mb-8">
          ...
          <p className={`mt-2 text-sm sm:text-base ${textColor} text-center`}>Let's personalize your learning experience.</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
          <div>
            <label htmlFor="userName" className={`block text-sm font-medium ${labelColor} mb-1.5`}>What's your name?</label>
            <input
              type="text"
              id="userName"
              value={userNameInput}
          ...
          <div>
            <label htmlFor="studyLevel" className={`block text-sm font-medium ${labelColor} mb-1.5`}>What's your current study level?</label>
            <select
              id="studyLevel"
              value={selectedLevel}
          ...
          {isCollegeLevel && (
            <div className={`p-4 rounded-md space-y-4 ${subSectionBg} border ${subSectionBorder}`}>
              <p className={`block text-sm font-medium ${labelColor} -mb-2`}>Are you studying at a well-known Indian college?</p>
              <div className="flex flex-col xs:flex-row gap-3">
                {(['yes', 'no'] as const).map(option => (
                  <label key={option} className={`flex-1 flex items-center p-3 border ${radioBorderColor} rounded-lg cursor-pointer transition-all hover:opacity-80 ${isWellKnownCollege === option ? `${radioCheckedBorderColor} ring-1 ${radioRingColor}` : ''}`}>
                    <input type="radio" name="isWellKnownCollege" value={option} checked={isWellKnownCollege === option} onChange={() => handleWellKnownChange(option)} className="h-4 w-4 text-indigo-600 border-slate-400 focus:ring-indigo-500 dark:text-sky-500 dark:border-slate-500 dark:focus:ring-sky-500 mr-2.5" />
              ...
              {isWellKnownCollege === 'yes' && (
                <div className="relative">
                  <label htmlFor="collegeName" className={`block text-sm font-medium ${labelColor} mb-1.5`}>Select your college:</label>
                  <div className="relative">
                    <input
                      type="text"
                      id="collegeName"
                      value={collegeSearchTerm}
                      onChange={(e) => { setCollegeSearchTerm(e.target.value); if (finalSelectedCollege !== e.target.value) setFinalSelectedCollege(''); setIsCollegeDropdownOpen(true); }}
                    ...
                    />
                    <ChevronDownIcon className={`absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 dark:text-slate-500 transition-transform ${isCollegeDropdownOpen ? 'rotate-180' : ''}`} />
                  </div>
                  {isCollegeDropdownOpen && (
                    <ul className={`absolute z-10 w-full mt-1 ${dropdownBg} border border-slate-200 dark:border-slate-600 rounded-md shadow-lg max-h-60 overflow-auto text-sm`}>
                      {filteredColleges.length > 0 ? filteredColleges.map(college => (
                        <li key={college} onMouseDown={() => { setFinalSelectedCollege(college); setCollegeSearchTerm(college); setIsCollegeDropdownOpen(false); }} className={`p-3 cursor-pointer ${textColor} ${dropdownItemHoverBg} transition-colors`}>
                          {college}
                        </li>
                      )) : (
                        <li className={`p-3 text-center text-slate-400 dark:text-slate-500`}>No matches found</li>
                      )}
                    </ul>
                  )}
                </div>
          ...
          {canAskExamQuestion && (
             <div className={`p-4 rounded-md space-y-4 ${subSectionBg} border ${subSectionBorder}`}>
                <div>
                    <p className={`block text-sm font-medium ${labelColor} mb-2`}>Are you preparing for any major exams?</p>
                    <div className="flex flex-col xs:flex-row gap-3">
                        {(['yes', 'no'] as const).map(option => (
                        <label key={option} className={`flex-1 flex items-center p-3 border ${radioBorderColor} rounded-lg cursor-pointer transition-all hover:opacity-80 ${isPreparingForExam === option ? `${radioCheckedBorderColor} ring-1 ${radioRingColor}` : ''}`}>
                            <input type="radio" name="isPreparingForExam" value={option} checked={isPreparingForExam === option} onChange={() => setIsPreparingForExam(option)} className="h-4 w-4 text-indigo-600 border-slate-400 focus:ring-indigo-500 dark:text-sky-500 dark:border-slate-500 dark:focus:ring-sky-500 mr-2.5" />
                ...
                {isPreparingForExam === 'yes' && (
                    <div className="space-y-4 pt-2">
                        <p className={`block text-sm font-medium ${labelColor} mb-2`}>Is it a general final exam, or a specific competitive exam?</p>
                         <div className="flex flex-col xs:flex-row gap-3">
                        ...
                        {examTypeCompetitive === 'no' && (
                             <div>
                                <label htmlFor="generalExamDescription" className={`block text-sm font-medium ${labelColor} mt-3 mb-1.5`}>Briefly describe your final exams (optional):</label>
                                <input
                                type="text"
                        ...
                        {examTypeCompetitive === 'yes' && (
                            <div className="relative mt-3">
                                <label htmlFor="competitiveExam" className={`block text-sm font-medium ${labelColor} mb-1.5`}>Select Competitive Exam:</label>
                                <div className="relative">
                                <input
                                    type="text"
                                    id="competitiveExamSearch"
                                    value={examSearchTerm}
                                    onChange={(e) => { setExamSearchTerm(e.target.value); if (selectedCompetitiveExamKey && POPULAR_INDIAN_EXAMS.find(ex => ex.key === selectedCompetitiveExamKey)?.name !== e.target.value) { setSelectedCompetitiveExamKey(''); } setIsExamDropdownOpen(true); }}
                                    onFocus={() => setIsExamDropdownOpen(true)}
                                    onBlur={() => setTimeout(() => setIsExamDropdownOpen(false), 200)}
                                    placeholder="Search or select exam..."
                                    className={`w-full border ${inputBg} ${inputTextColor} rounded-lg p-3 focus:ring-2 focus:outline-none transition-colors text-base pr-10`}
                                />
                                <ChevronDownIcon className={`absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 dark:text-slate-500 transition-transform ${isExamDropdownOpen ? 'rotate-180' : ''}`} />
                                </div>
                                {isExamDropdownOpen && (
                                <ul className={`absolute z-20 w-full mt-1 ${dropdownBg} border border-slate-200 dark:border-slate-600 rounded-md shadow-lg max-h-60 overflow-y-auto text-sm`}>
                                    {Object.keys(groupedExams).length > 0 ? Object.entries(groupedExams).map(([groupName, examsInGroup]) => (
                                    <React.Fragment key={groupName}>
                                        <li className={`px-3 py-1.5 text-xs font-semibold text-slate-500 bg-slate-100 dark:text-slate-400 dark:bg-slate-700/50 sticky top-0`}>{groupName}</li>
                                        {examsInGroup.map(exam => (
                                        <li key={exam.key} onMouseDown={() => handleExamSelection(exam.key)} className={`p-3 cursor-pointer ${textColor} ${dropdownItemHoverBg} transition-colors`}>
                                            {exam.name}
                                        </li>
                                        ))}
                                    </React.Fragment>
                        ...
                        {examTypeCompetitive === 'yes' && selectedCompetitiveExamKey === EXAM_PREP_OTHER_KEY && (
                            <div className="mt-3">
                                <label htmlFor="otherCompetitiveExamName" className={`block text-sm font-medium ${labelColor} mb-1.5`}>Please specify the exam name:</label>
                                <input
                                type="text"
          ...
          <div className="pt-3">
            <button
              ...
              className={`w-full ${buttonBg} text-white font-semibold py-3.5 px-6 rounded-lg shadow-lg hover:shadow-xl transition-all duration-200 ease-in-out flex items-center justify-center gap-2 text-base focus:outline-none focus:ring-2 ${buttonFocusRing} focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-slate-800 hover:scale-[1.02] hover:-translate-y-0.5 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
          ...
          <div className="pt-3">
            <button
              ...
              className={`w-full bg-gray-500 hover:bg-gray-600 text-white font-semibold py-3.5 px-6 rounded-lg shadow-lg hover:shadow-xl transition-all duration-200 ease-in-out flex items-center justify-center gap-2 text-base focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-slate-800 hover:scale-[1.02] hover:-translate-y-0.5`}
            >
              Logout
            </button>
          </div>
